...
class User {
    ...
    deposit(amount: number): void {
        if (amount > 0) {
            this.balance += amount;
            const transaction = `Deposited $${amount.toFixed(2)} on ${format(new Date(), 'yyyy-MM-dd HH:mm:ss')}`;
            this.transactionHistory.push(transaction);
            console.log(`$${amount.toFixed(2)} deposited successfully. Current balance: $${this.balance.toFixed(2)}`);
        } else {
    ...
    transfer(recipient: User, amount: number): void {
        if (this.balance >= amount && amount > 0) {
            ...
            this.transactionHistory.push(`Transferred $${amount.toFixed(2)} to ${recipient.username} on ${timestamp}`);
            recipient.transactionHistory.push(`Received $${amount.toFixed(2)} from ${this.username} on ${timestamp}`);

            console.log(`Successfully transferred $${amount.toFixed(2)} to ${recipient.username}. Current balance: $${this.balance.toFixed(2)}`);
        } else {
